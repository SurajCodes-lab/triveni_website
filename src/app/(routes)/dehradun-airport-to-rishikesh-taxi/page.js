...
export const metadata = {
  ...
  keywords: 'dehradun airport to rishikesh taxi, jolly grant airport rishikesh cab, dehradun to rishikesh transfer, rishikesh airport taxi, yoga capital taxi',
  ...
  openGraph: {
    title: `Dehradun Airport to Rishikesh Taxi | Fixed Fare \u20B9${data.fares.sedan.price}`,
    description: `Book taxi from Jolly Grant Airport to Rishikesh. Fastest transfer to Yoga Capital. ${data.time}.`,
    type: 'website', locale: 'en_IN', url: `https://www.trivenicabs.in/${SLUG}`, siteName: 'Triveni Cabs',
  },
  twitter: { card: 'summary_large_image', title: `Dehradun Airport to Rishikesh | \u20B9${data.fares.sedan.price}`, description: 'Jolly Grant to Rishikesh. Quick transfer to Yoga Capital.' },
...
export default function Page() {
  ...
  const serviceSchema = { "@context": "https://schema.org", "@type": "Service", "serviceType": "Airport Transfer", "name": `${data.airportCity} Airport to ${data.destination} Taxi Service`, "description": `Reliable taxi from ${data.airport} to ${data.destination}. Fastest airport transfer to Rishikesh.`, "provider": { "@type": "LocalBusiness", "name": "Triveni Cabs", "telephone": "+91-7668570551", "address": { "@type": "PostalAddress", "addressLocality": "Agra", "postalCode": "282006", "addressCountry": "IN" } }, "offers": { "@type": "AggregateOffer", "lowPrice": data.fares.sedan.price.replace(',', ''), "highPrice": data.fares.tempo.price.replace(',', ''), "priceCurrency": "INR" }, "areaServed": { "@type": "Place", "name": `${data.airport} to ${data.destination}` } };
