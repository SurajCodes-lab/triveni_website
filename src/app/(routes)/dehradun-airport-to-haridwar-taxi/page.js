...
export const metadata = {
  ...
  keywords: 'dehradun airport to haridwar taxi, jolly grant airport haridwar cab, dehradun to haridwar transfer, haridwar airport taxi',
  ...
  openGraph: {
    title: `Dehradun Airport to Haridwar Taxi | Fixed Fare \u20B9${data.fares.sedan.price}`,
    description: `Book taxi from Jolly Grant Airport to Haridwar. Nearest airport, quick transfer. ${data.time}.`,
    type: 'website', locale: 'en_IN', url: `https://www.trivenicabs.in/${SLUG}`, siteName: 'Triveni Cabs',
  },
  twitter: { card: 'summary_large_image', title: `Dehradun Airport to Haridwar | \u20B9${data.fares.sedan.price}`, description: 'Jolly Grant Airport to Haridwar. Quick pilgrimage transfer.' },
...
export default function Page() {
  ...
  const serviceSchema = { "@context": "https://schema.org", "@type": "Service", "serviceType": "Airport Transfer", "name": `${data.airportCity} Airport to ${data.destination} Taxi Service`, "description": `Reliable taxi from ${data.airport} to ${data.destination}. Fixed fare, Ganga Aarti coordination.`, "provider": { "@type": "LocalBusiness", "name": "Triveni Cabs", "telephone": "+91-7668570551", "address": { "@type": "PostalAddress", "addressLocality": "Agra", "postalCode": "282006", "addressCountry": "IN" } }, "offers": { "@type": "AggregateOffer", "lowPrice": data.fares.sedan.price.replace(',', ''), "highPrice": data.fares.tempo.price.replace(',', ''), "priceCurrency": "INR" }, "areaServed": { "@type": "Place", "name": `${data.airport} to ${data.destination}` } };
