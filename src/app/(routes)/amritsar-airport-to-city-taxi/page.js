...
export const metadata = {
  ...
  keywords: 'amritsar airport to city taxi, amritsar airport cab, amritsar airport golden temple taxi, amritsar airport transfer, NRI taxi amritsar',
  ...
  openGraph: {
    title: `Amritsar Airport to City Taxi | Fixed Fare \u20B9${data.fares.sedan.price} | Golden Temple`,
    description: `Book taxi from Amritsar Airport to city. Golden Temple direct drop, NRI-friendly service. ${data.time}.`,
    type: 'website', locale: 'en_IN', url: `https://www.trivenicabs.in/${SLUG}`, siteName: 'Triveni Cabs',
  },
  twitter: { card: 'summary_large_image', title: `Amritsar Airport to City | \u20B9${data.fares.sedan.price}`, description: 'Amritsar Airport to Golden Temple. NRI-friendly fixed fare.' },
...
export default function Page() {
  ...
  const serviceSchema = { "@context": "https://schema.org", "@type": "Service", "serviceType": "Airport Transfer", "name": `${data.airportCity} Airport to ${data.destination} Taxi Service`, "description": `Reliable taxi from ${data.airport} to ${data.destination}. Golden Temple direct drop, NRI-friendly.`, "provider": { "@type": "LocalBusiness", "name": "Triveni Cabs", "telephone": "+91-7668570551", "address": { "@type": "PostalAddress", "addressLocality": "Agra", "postalCode": "282006", "addressCountry": "IN" } }, "offers": { "@type": "AggregateOffer", "lowPrice": data.fares.sedan.price.replace(',', ''), "highPrice": data.fares.tempo.price.replace(',', ''), "priceCurrency": "INR" }, "areaServed": { "@type": "Place", "name": `${data.airport} to ${data.destination}` } };
