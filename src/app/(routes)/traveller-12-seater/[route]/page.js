...
const VEHICLE_TYPE = 'traveller-12-seater';
const VEHICLE_SLUG = 'traveller-12-seater';
...
export async function generateMetadata({ params }) {
  ...
  const description = `Hire ${vehicleSpec.name} (${vehicleSpec.type}, ${vehicleSpec.seating} seater) from ${routeData.origin} to ${routeData.destination} at Rs ${routeData.fare.toLocaleString('en-IN')}. ${routeData.distanceKm} km, ${routeData.duration}. AC, experienced driver. Call 7668570551.`;
  const ogTitle = `${vehicleSpec.name} ${vehicleSpec.seating} Seater: ${routeData.origin} to ${routeData.destination} — Rs ${routeData.fare.toLocaleString('en-IN')} All-Inclusive | Triveni Cabs`;
  ...
  return {
    ...
    keywords: [
      ...
      `12 seater tempo traveller ${routeData.origin.toLowerCase()} to ${routeData.destination.toLowerCase()}`,
      ...
      `tempo traveller on rent ${routeData.origin.toLowerCase()}`,
      `hire tempo traveller for ${routeData.destination.toLowerCase()}`,
      `${routeData.origin.toLowerCase()} to ${routeData.destination.toLowerCase()} by tempo traveller`,
      `12 seater booking ${routeData.origin.toLowerCase()}`,
      `tempo traveller hire ${routeData.origin.toLowerCase()} to ${routeData.destination.toLowerCase()}`,
      `${routeData.origin.toLowerCase()} to ${routeData.destination.toLowerCase()} group travel`,
      `tempo traveller outstation ${routeData.origin.toLowerCase()}`,
      `best tempo for ${routeData.destination.toLowerCase()} trip`,
    ],
...
export default async function VehicleHirePage({ params }) {
  ...
  const serviceSchema = {
    ...
    "@type": "Service",
    "serviceType": `${vehicleSpec.name} ${vehicleSpec.seating} Seater Hire ${routeData.origin} to ${routeData.destination}`,
    "name": `${vehicleSpec.name} Hire: ${routeData.origin} to ${routeData.destination}`,
    "description": `Hire ${vehicleSpec.name} ${vehicleSpec.seating} seater from ${routeData.origin} to ${routeData.destination}. ${routeData.distanceKm} km, ${routeData.duration}. All-inclusive fare Rs ${routeData.fare.toLocaleString('en-IN')}.`,
    "provider": {
      "@type": "LocalBusiness",
      "name": "Triveni Cabs",
    ...
    "areaServed": [{ "@type": "City", "name": routeData.origin }, { "@type": "City", "name": routeData.destination }],
    "offers": { "@type": "Offer", "price": routeData.fare.toString(), "priceCurrency": "INR", "availability": "https://schema.org/InStock" },
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": `${vehicleSpec.name} ${vehicleSpec.seating} Seater`, "item": `https://www.trivenicabs.in/${VEHICLE_SLUG}` },
